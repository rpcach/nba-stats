...
get_boxscore_traditional <- function(game_id) {
	...
	if(as.numeric(substring(game_id,4,5)) > 20) {
		return(NULL)
	}
...
#adds data to db for season. stops when 10 errors reach (meaning no more games)
new_loop <- function() {
	year <- 1988
	year <- substring(year,3,4)
	error_count <- 0
	...
	while(error_count < 10) {
		i <- i + 1
		game_id <- c("002", year, rep("0",5-nchar(i)), i)
		game_id <- paste(game_id,collapse='')
		print(i)
		tryCatch(get_boxscore_traditional(game_id), error=function(e) {Sys.sleep(1);error_count <<- error_count + 1})
	}
}
